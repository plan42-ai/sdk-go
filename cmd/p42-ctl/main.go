...
func loadFeatureFlags(s *SharedOptions, f *p42.FeatureFlags) error {
...
func dispatchCommand(kongctx *kong.Context, options *Options) error {
	...
	case "github list-orgs-for-connection":
		return options.Github.ListOrgsForConnection.Run(options.Ctx, &options.SharedOptions)
	...
	case "runner delete-queue":
		return options.Runner.DeleteQueue.Run(options.Ctx, &options.SharedOptions)
...
func postProcessOptions(o *Options) error {
	...
	options = append(options, p42.WithSigv4Auth(awsCfg, clock.RealClock{}))
	o.Client = p42.NewClient(o.Endpoint, options...)
