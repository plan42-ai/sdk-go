		...
		http.HandlerFunc(
			func(w http.ResponseWriter, r *http.Request) {
				...
				case "/v1/tenants/tenant/workstreams/source/tasks/task":
					...
					w.Header().Set("Content-Type", "application/json")
					fmt.Fprintf(w, `{"TenantId":"tenant","TaskId":"task","Title":"","Prompt":"","Parallel":false,"RepoInfo":{},"State":"Pending","CreatedAt":"%s","UpdatedAt":"%s","Deleted":false,"Version":2}`, now, now)
				case "/v1/tenants/tenant/workstreams/source/tasks/task/move":
					...
					w.Header().Set("Content-Type", "application/json")
					fmt.Fprintf(w, `{"Task":{"TenantId":"tenant","TaskId":"task","Title":"","Prompt":"","Parallel":false,"RepoInfo":{},"State":"Pending","CreatedAt":"%s","UpdatedAt":"%s","Deleted":false,"Version":3},"SourceWorkstream":{},"DestinationWorkstream":{}}`, now, now)
				default:
